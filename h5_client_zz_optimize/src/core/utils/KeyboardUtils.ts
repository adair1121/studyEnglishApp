...
class KeyboardUtils extends BaseClass {
	...
	public constructor() {
		...
		document.addEventListener("keyup", function (e):void {
			for (let i:number = 0, len = self.key_ups.length; i < len; i++) {
				let func:Function = self.key_ups[i][0];
				let target:any = self.key_ups[i][1];
				if (target) {
		...
		document.addEventListener("keydown", function (e):void {
			for (let i:number = 0, len = self.key_downs.length; i < len; i++) {
				let func:Function = self.key_downs[i][0];
				let target:any = self.key_downs[i][1];
				if (target) {
					func.call(target, e["keyCode"]);
				} else {
					func(e["keyCode"]);
	...
	public static ins():KeyboardUtils{
		return super.ins() as KeyboardUtils;
	}
	/**
	 * 添加KeyUp事件
	 * @param callback 回调函数
	...
	public removeKeyUp(callback:Function, target:any):void {
		for (let i = 0; i < this.key_ups.length; i++) {
	...
	public removeKeyDown(callback:Function, target:any):void {
		for (let i = 0; i < this.key_downs.length; i++) {
