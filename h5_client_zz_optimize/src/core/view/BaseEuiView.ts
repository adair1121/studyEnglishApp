...
class BaseEuiView extends BaseView implements IBaseView {
    ...
    /**hide 除了第一个页签意外的页签显示 */
    public hidePageFunc(viewStack:eui.ViewStack,index:number | number[] = 0):void{
        let fixitem:BaseComponent;
        let fixitems:BaseComponent[] = [];
        if(typeof index == "number"){
           fixitem = <BaseComponent>viewStack.getChildAt(index);
        }else{
            for(let j:number = 0;j<(<number[]>index).length;j++){
                fixitem = <BaseComponent>viewStack.getChildAt(index[j]);
                fixitems.push(fixitem)
            }
            fixitem = null;
        }
        if(viewStack.$children.length >= 2){
            for(let i:number = 0;i<viewStack.$children.length;i++){
                let item:BaseComponent = <BaseComponent>viewStack.getChildAt(i);
                if(!!fixitem){
                    if(fixitem != item){
